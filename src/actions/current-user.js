...
function currentUserOnChangeHandlerGenerator(dispatch): func {
  const onChangeHandler = (latestUserSnapshot) => {
...
export function observeCurrentUserStart(currentUserId) {
  return async (dispatch) => {
    ...
    try {
      const onChange = currentUserOnChangeHandlerGenerator(dispatch)
...
export function observeCurrentUserFailure() {
  return { type: 'OBSERVE_CURRENT_USER_FAILURE' }
}
...
export function logoutStart(currentUserId: stirng): (dispatch: func) => Promise<void> {
  return async (dispatch): Promise<void> => {
    ...
    try {
      await signOut()
      const onChange: func = currentUserOnChangeHandlerGenerator(dispatch)
      unobserveUser(currentUserId, onChange)
    } catch (error) {
